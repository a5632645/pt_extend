...
#define pt_extend_nest_delay(ms)\
    do {\
        ...
        _pt_extend_unduplicate_label(&pt_extend::ptCallContext->pt_, PT_STATUS_BLOCKED);\
        if (pt_status(&pt_extend::GetCurrentTask()->pt_) == PT_STATUS_YIELDED) {\
            return;\
        }\
    } while(0)
...
#define pt_extend_nest_begin()\
    do {\
        pt_begin(&pt_extend::ptCallContext->pt_);\
    } while (0)
...
#define pt_extend_co_call(ptCallCtx, func, ...)\
    do {\
        ptCallCtx.prev_ = pt_extend::ptCallContext;\
        pt_extend::ptCallContext = &ptCallCtx;\
        pt_label(&pt_extend::GetCurrentTask()->pt_, PT_STATUS_BLOCKED);\
        ++pt_extend::nestingLevel;\
        func(__VA_ARGS__);\
        --pt_extend::nestingLevel;\
        if (pt_status(&pt_extend::ptCallContext->pt_) != PT_STATUS_FINISHED) {\
            return;\
        }\
...
/* 协程函数调用协程函数 */
#define pt_extend_nest_call(ptCallCtx, func, ...)\
    do {\
        ptCallCtx.prev_ = pt_extend::ptCallContext;\
        pt_label(&pt_extend::ptCallContext->pt_, PT_STATUS_BLOCKED);\
        pt_extend::ptCallContext = &ptCallCtx;\
        ++pt_extend::nestingLevel;\
        func(__VA_ARGS__);\
        --pt_extend::nestingLevel;\
